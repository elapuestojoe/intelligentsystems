...
class PicAPix(CSP):
    ...
    def assign(self, var, val, assignment):
        ...
        #     print(self.confs)
        #     print(assignment)
        #     print(self.nconflicts(var, val, assignment))
    ...
    def nconflicts(self, var, val, assignment):
        ...
        if var in col1:
            ...
            conflicts += abs(1 - countOcurrences(firstColumn, "V"))
            conflicts += countOcurrences(firstColumn, "R")
            # conflicts += abs(4 - countOcurrences(firstColumn, "."))
        # Constraint 2

        elif var in col2:
            secondColumn = getAssignmentColumn(assignment, 1, 21, 5)
            conflicts += abs(1 - countOcurrences(secondColumn, "V"))
            # conflicts += abs(4 - countOcurrences(secondColumn, "."))

        elif var in col3:
            thirdColumn = getAssignmentColumn(assignment, 2, 22, 5)
            conflicts += abs(2 - countOcurrences(thirdColumn, "V"))
            conflicts += abs(1 - countOcurrences(thirdColumn, "R"))
            # conflicts += abs(2 - countOcurrences(thirdColumn, "."))

        elif var in col4:
            fourthColumn = getAssignmentColumn(assignment, 3, 23, 5)
            conflicts += abs(2 - countOcurrences(fourthColumn, "R"))
            conflicts += abs(1 - countOcurrences(fourthColumn, "A"))
            # conflicts += abs(2 - countOcurrences(fourthColumn, "."))

        else:
            fifthColumn = getAssignmentColumn(assignment, 4, 24, 5)
            conflicts += abs(1 - countOcurrences(fifthColumn, "R"))
            # conflicts += abs(4 - countOcurrences(fifthColumn, "."))


        # Horizontal
        if var in row1:
            firstRow = getAssignmentColumn(assignment, 0, 4, 1)
            conflicts += abs(1 - countOcurrences(firstRow, "R"))
            # conflicts += abs(4 - countOcurrences(firstRow, "."))

        elif var in row2:
            secondRow = getAssignmentColumn(assignment, 5, 9, 1)
            conflicts += abs(2 - countOcurrences(secondRow, "R"))
            conflicts += abs(1 - countOcurrences(secondRow, "A"))
            # conflicts += abs(2 - countOcurrences(secondRow, "."))

        elif var in row3:
            thirdRow = getAssignmentColumn(assignment, 10, 14, 1)
            conflicts += abs(1 - countOcurrences(thirdRow, "R"))
            # conflicts += abs(4 - countOcurrences(thirdRow, "."))

        elif var in row4:
            fourthRow = getAssignmentColumn(assignment, 15, 19, 1)
            conflicts += abs(2 - countOcurrences(fourthRow, "V"))
            # conflicts += abs(3 - countOcurrences(fourthRow, "."))

        else:
            fifthRow = getAssignmentColumn(assignment, 20, 24, 1)
            conflicts += abs(2 - countOcurrences(fifthRow, "V"))
    ...
    def pic_a_pix_constraint_h(self, A, a, B, b):
        ...
        # if( (A == 3 or A == 23) and a == "A"):
        #     return False
        # if( (B == 3 or B == 23) and b == "A"):
        #     return False

        # if( (A in [17,22] ) and a == "R"):
        #     return False

        # if( (B in [17,22] ) and b == "R"):
        #     return False
        ...
        # if(A in [5,6,7,8,9] and B in [5,6,7,8,9]):
        #     if(a == "R" and b == "R"):
        #         return abs(A-B)>1
        ...
        # if(B == 3 and b == "V"):
        #     return False
        # if(A in [3,8,13,18,23] and B in [3,8,13,18,23]):
        #     if(a == "R" and b == "R"):
        #         return abs(A-B)>1

        # if(A in [8,13,18,23] and B in [8,13,18,23]):
        #     if(a == "V" and b == "V"):
        #         return abs(A-B)==1
        
        # if(A in [20,21,22,23,24] and B in [20,21,22,23,24]):
        #     if(a == "V" and b == "V"):
        #         return abs(A-B)==1
...
# # print(AC3(s))
# print(backtracking_search(s, select_unassigned_variable=mrv, inference=forward_checking))
# s.display(s.infer_assignment())